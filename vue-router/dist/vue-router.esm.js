 ...
 * @LastEditors: jackson
 * @LastEditTime: 2019-10-16 15:35:58
 */
 ...
 * @Date: 2019-07-10 10:30:54
 * @LastEditors: jackson
 * @LastEditTime: 2019-10-16 14:12:44
 * @Desc：此文件重要的做了一下几件事：
...
function install (Vue) {
  ...
  var isDef = function (v) { return v !== undefined; }; // 检查变量是否是undefined, isDef函数返回的是一个boolean值

  var registerInstance = function (vm, callVal) {
    var i = vm.$options._parentVnode; // 获取vm的父级虚拟dom
...
function createRouteMap (
  ...
  var pathList = oldPathList || []; // oldPathList不存在时，为空数组
  // $flow-disable-line
  var pathMap = oldPathMap || Object.create(null); // oldPathMap不存在时为空对象
  // $flow-disable-line
  var nameMap = oldNameMap || Object.create(null); // oldNameMap不存在时为空对象
...
var supportsPushState = inBrowser && (function () {
  var ua = window.navigator.userAgent; // 获取客户端的代理对象
...
function getLocation (base) {
  var path = decodeURI(window.location.pathname); // 获取路径并解码
...
var VueRouter = function VueRouter (options) {
  ...
  var mode = options.mode || 'hash'; // 路由模式，hash、history、abstract，默认为hash模式
...
VueRouter.prototype.init = function init (app /* Vue component instance */) {
  ...
  var history = this.history; // 存储路由模式
