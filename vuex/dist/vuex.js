...
(function (global, factory) {
   ...
   * @LastEditors: jackson
   * @LastEditTime: 2019-10-23 15:53:04
   */
  function applyMixin (Vue) {
    ...
    function vuexInit () {
      var options = this.$options; // 获取vue的配置选项
      // store injection 
      // 根实例是否有store配置
      if (options.store) {
        // 给根实例挂载$store属性
   ...
   * @Date: 2019-10-16 17:00:14
   * @LastEditors: jackson
   * @LastEditTime: 2019-10-23 17:27:50
   */
   ...
   * @Date: 2019-10-16 17:00:14
   * @LastEditors: jackson
   * @LastEditTime: 2019-10-23 18:46:28
   */
  ...
  ModuleCollection.prototype.get = function get (path) {
    var a = path.reduce(function (module, key) {
  ...
  ModuleCollection.prototype.register = function register (path, rawModule, runtime) {
    ...
    var newModule = new Module(rawModule, runtime); // 每一个模块都是重新实例化的模块对象
    if (path.length === 0) { // 没有子模块时
      this.root = newModule;
    } else {
      var a = path.slice(0, -1);
      var parent = this.get(a);
      parent.addChild(path[path.length - 1], newModule);
    }

    // register nested modules
    if (rawModule.modules) { // 有子模块时
      forEachValue(rawModule.modules, function (rawChildModule, key) { // 获取子模块的key并遍历，传入value值和key
        this$1.register(path.concat(key), rawChildModule, runtime); // 递归调用register进行子模块的注册
      });
    }
  };
